...
[更多命令参见官网](https://redis.io/commands)
...
Redisson是一个企业级的开源Redis Client，这个推荐使用。且这个Client还带有分布式锁支持，从而避免使用原生Client API实现分布式锁时考虑的一些复杂问题。

# 其他闲杂

（1）缓存击穿，缓存穿透

（2）从Redis 2.6版本起，Redis开始支持Lua脚本，一个Lua脚本操作Redis，是一个原子性操作，Redis分布式的实现一般来说会用到Lua脚本。
